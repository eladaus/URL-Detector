...
public class InfiniteLoopPreventionTests
{
    ...
    {
        var sourceText =
            " :u "; // previously, without character read count logic, this string kills loop in UrlDetector.ReadDefault()
    ...
    [Theory]
    [MemberData(nameof(DataForLinkedInUrlParserFlags))]
    public void TestSingleLevelDomainLocalhostWithPortUrlDetectionAfterMultipleColons(
        UrlDetectorOptions flags
    )
    {
        var sourceText =
            " l:url(https://localhost:8080/test "; // previously, without character read count logic, this string kills loop in UrlDetector.ReadDefault()
    ...
    {
        var sourceText =
            " l:url(https://test/test2 "; // previously, without character read count logic, this string kills loop in UrlDetector.ReadDefault()
        var links = new UrlDetector(sourceText, flags)
            // BEWARE THE INFINITE LOOP THAT HAPPENS HERE:
            .Detect();
        Assert.True(true); // Just want to confirm that we made it past the infinite loop
    ...
    [Theory]
    [MemberData(nameof(DataForLinkedInUrlParserFlags))]
    public void TestSingleLevelDomainUrlWithPortDetectionAfterMultipleColons(
        UrlDetectorOptions flags
    )
    {
        var sourceText =
            " l:url(https://test:8080/test "; // previously, without character read count logic, this string kills loop in UrlDetector.ReadDefault()
        var links = new UrlDetector(sourceText, flags)
            // BEWARE THE INFINITE LOOP THAT HAPPENS HERE:
            .Detect();
        Assert.True(true); // Just want to confirm that we made it past the infinite loop

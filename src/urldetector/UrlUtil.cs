...
namespace urldetector
{
    public class UrlUtil
    {
            ...
            while (i < stringBuilder.Length - 2)
            {
                ...
                if (curr == '%')
                {
                    if (
                        ...
                        && CharUtils.IsHex(stringBuilder[i + 2])
                    )
                    {
                        var startIndex = i + 1;
                        var endIndex = i + 3;
                        var decodedChar = (char)
                            Convert.ToInt16(
                                stringBuilder.ToString(startIndex, endIndex - startIndex),
                                16
                            );
        ...
        /// @param urlPart The part of the url we are making canonical
        /// </summary>
        ...
        {
            var encoder = new StringBuilder(url.Length);

    ...
    public class DomainNameReader
    {
        ...
        private ReaderNextState ReadCurrent()
        {
            if (_current != null)
            {
                ...
                //hex special case
                var isAllHexSoFar =
                    length > 2 && _current[0] == '0' && (_current[1] == 'x' || _current[1] == 'X');
                ...
                while (index < length && !done)
                {
                    ...
                    else if (curr == '[')
                    {
                        ...
                        curr == '%'
                        && index + 2 < length
                        && CharUtils.IsHex(_current[index + 1])
                    ...
                    )
                    {
                        ...
                        !CharUtils.IsNumeric(curr)
                        && !_options.HasFlag(UrlDetectorOptions.ALLOW_SINGLE_LEVEL_DOMAIN)
                    )
                    {
        ...
        private ReaderNextState CheckDomainNameValid(ReaderNextState validState, char? lastChar)
        {
            ...
            var lastDotLength =
                _buffer.Length > 3
                && _buffer
                    .ToString(_buffer.Length - 3)
                    .Equals(PERCENT_HEX_ENCODED_DOT, StringComparison.OrdinalIgnoreCase)
                    ? 3
                    : 1;

            var domainLength =
                _buffer.Length - _startDomainName + (_currentLabelLength > 0 ? lastDotLength : 0);
            ...
            else if (_seenBracket)
            {
                ...
                _currentLabelLength > 0 && _dots >= 1
                || _dots >= 2 && _currentLabelLength == 0
            ...
            if (testDomain.Length > 0)
            {
                ...
                if (_dots == 0)
                {
                    try
                    {
                        ...
                        valid =
                            value <= MAX_NUMERIC_DOMAIN_VALUE && value >= MIN_NUMERIC_DOMAIN_VALUE;
                    }
                    catch (Exception)
                    {
                        valid = false;
                        ...
                        if (part.Length > 0)
                        {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        if (
                                            !OctalEncodingHelper.TryParseOctal(
                                                parsedNum,
                                                out section
                                            )
                                        )
                ...
                switch (testDomain[index])
                {
                    ...
                    case ']': //found end of ipv6 address
                        if (testDomain[index] == '%')
                        {
                            ...
                            if (
                                testDomain.Length - index >= 2
                                && testDomain[index + 1] == '2'
                                && testDomain[index + 2] == 'e'
                            )
                            {

...
public class UrlDetector
{
    ...
    public UrlDetector(
        ...
        HashSet<string> validSchemes = null
    )
    {
        ...
        while (!_reader.Eof())
        {
            ...
            switch (curr)
            {
                ...
                case '%':
                    ...
                    {
                        if (_reader.PeekSpan(2).Equals("3a", StringComparison.OrdinalIgnoreCase))
                        {
                            _buffer.Append(curr);
    ...
    private CharacterMatch CheckMatchingCharacter(char curr)
    {
        ...
        if (
            (curr == '\"' && _options.HasFlag(UrlDetectorOptions.QUOTE_MATCH))
        ...
        )
        {
            ...
            return quoteStart || currVal % 2 == 0
                ? CharacterMatch.CharacterMatchStop
                : CharacterMatch.CharacterMatchStart;
        }

        if (
            _options.HasFlag(UrlDetectorOptions.BRACKET_MATCH)
            && (curr == '[' || curr == '{' || curr == '(')
        )
        {
    ...
    private bool ReadDomainName(string current)
    {
        ...
        var reader = new DomainNameReader(
            _reader,
            _buffer,
            current,
            _options,
            c => CheckMatchingCharacter(c)
        );
        ...
        switch (state)
        {
            ...
            case DomainNameReader.ReaderNextState.ReadUserPass:
                var host = _currentUrlMarker.IndexOf(UrlPart.HOST);
    ...
    private bool ReadPort()
    {
        ...
        while (!_reader.Eof())
        {
            ...
            if (
                CheckMatchingCharacter(curr) == CharacterMatch.CharacterMatchStop

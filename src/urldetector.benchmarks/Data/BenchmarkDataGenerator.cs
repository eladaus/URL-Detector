...
namespace urldetector.benchmarks.Data
{
    ...
    public static class BenchmarkDataGenerator
    {
        ...
        )
        {
            ...
            {
                bool insertUrl =
                    profile != UrlMixProfile.NoUrls && random.NextDouble() < urlDensity;
        ...
        )
        {
            ...
            while (sb.Length < approximateSizeBytes)
            {
                ...
                bool insertUrl = random.NextDouble() < urlDensity;

                if (insertUrl)
                {
                    string url = PickUrl(random, UrlMixProfile.WebOnly);
                    string template = HtmlUrlTemplates[random.Next(HtmlUrlTemplates.Length)];
                    sb.AppendLine(string.Format(template, url));
                }
                else
                {
                    string filler = FillerText[random.Next(FillerText.Length)];
                    string template = HtmlFillerTemplates[random.Next(HtmlFillerTemplates.Length)];
        ...
        )
        {
            ...
            while (sb.Length < approximateSizeBytes)
            {
                ...
                {
                    string url = PickUrl(random, UrlMixProfile.WebOnly);
                    string template = JsonUrlEntryTemplates[
                        random.Next(JsonUrlEntryTemplates.Length)
                    ];
                    sb.Append(string.Format(template, url, filler));
                }
                else
                {
                    string template = JsonFillerEntryTemplates[
        ...
        )
        {
            ...
            while (sb.Length < approximateSizeBytes)
            {
                ...
                {
                    string url = PickUrl(random, UrlMixProfile.WebOnly);
                    string template = XmlUrlEntryTemplates[
                        random.Next(XmlUrlEntryTemplates.Length)
                    ];
                    sb.AppendLine(string.Format(template, url, filler));
                }
                else
                {
                    string template = XmlFillerEntryTemplates[
                        random.Next(XmlFillerEntryTemplates.Length)
                    ];
        ...
        )
        {
            ...
            while (sb.Length < approximateSizeBytes)
            {
                ...
                {
                    string url = PickUrl(random, UrlMixProfile.WebOnly);
                    string template = JsUrlStatementTemplates[
                        random.Next(JsUrlStatementTemplates.Length)
                    ];
                    sb.AppendLine(string.Format(template, url, filler));
                }
                else
                {
                    string template = JsFillerStatementTemplates[

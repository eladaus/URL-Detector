...
{
	public class LegacyUrlDetector
	{
		/// <summary>
		...
		/// </summary>
		private LegacyUrlMarker _currentUrlMarker = new LegacyUrlMarker();
		...
		/// </summary>
		private readonly List<LegacyUrl> _urlList = new List<LegacyUrl>();
		...
		/// <param name="validSchemes"></param>
		public LegacyUrlDetector(string content, LegacyUrlDetectorOptions options, HashSet<string> validSchemes = null)
		{
			_reader = new LegacyInputTextReader(content);
			_options = options;
			...
			while (!_reader.Eof())
			{
				...
				switch (curr)
				{
					case ' ':
						...
						if (_options.HasFlag(LegacyUrlDetectorOptions.ALLOW_SINGLE_LEVEL_DOMAIN) && _buffer.Length > 0 && _hasScheme)
						{
							_reader.GoBack();
					...
					case '%':
						if (_reader.CanReadChars(2))
						{
							...
							else if (LegacyCharUtils.IsHex(_reader.PeekChar(0)) && LegacyCharUtils.IsHex(_reader.PeekChar(1)))
							{
								_buffer.Append(curr);
								_buffer.Append(_reader.Read());
								_buffer.Append(_reader.Read());
					...
					case '@': //Check the domain name after a username
						if (_buffer.Length > 0)
						{
							_currentUrlMarker.SetIndex(LegacyUrlPart.USERNAME_PASSWORD, length);
							_buffer.Append(curr);
					...
					case '/':
						...
						if (_hasScheme || _options.HasFlag(LegacyUrlDetectorOptions.ALLOW_SINGLE_LEVEL_DOMAIN) && _buffer.Length > 1)
						{
		...
		private int ProcessColon(int length)
		{
			...
			else if (_buffer.Length > 0 && _options.HasFlag(LegacyUrlDetectorOptions.ALLOW_SINGLE_LEVEL_DOMAIN)
		...
		private CharacterMatch CheckMatchingCharacter(char curr)
		{
			...
			if (curr == '\"' && _options.HasFlag(LegacyUrlDetectorOptions.QUOTE_MATCH)
			    || curr == '\'' && _options.HasFlag(LegacyUrlDetectorOptions.SINGLE_QUOTE_MATCH))
			{
				bool quoteStart;
				if (curr == '\"')
				{
			...
			if (_options.HasFlag(LegacyUrlDetectorOptions.BRACKET_MATCH) && (curr == '[' || curr == '{' || curr == '('))
			{
			...
			if (_options.HasFlag(LegacyUrlDetectorOptions.XML) && curr == '<')
			{
			...
			if (_options.HasFlag(LegacyUrlDetectorOptions.BRACKET_MATCH) && (curr == ']' || curr == '}' || curr == ')')
			    || _options.HasFlag(LegacyUrlDetectorOptions.XML) && curr == '>')
			{
			...
			while (!_reader.Eof())
			{
				...
				if (curr == '/')
				{
					...
					if (numSlashes == 1)
					{
						...
						{
							_currentUrlMarker.SetIndex(LegacyUrlPart.SCHEME, 0);
							return true;
						}
				...
				else if (originalLength > 0 || numSlashes > 0 || !LegacyCharUtils.IsAlpha(curr))
				{
		...
		private bool ReadUserPass(int beginningOfUsername)
		{
			...
			while (!done && !_reader.Eof())
			{
				...
				{
					_buffer.Append(curr);
					_currentUrlMarker.SetIndex(LegacyUrlPart.USERNAME_PASSWORD, beginningOfUsername);
					return ReadDomainName("");
				}

				if (LegacyCharUtils.IsDot(curr) || curr == '[')
				{
		...
		private bool ReadDomainName(string current)
		{
			...
			_currentUrlMarker.SetIndex(LegacyUrlPart.HOST, hostIndex);
			...
			var reader = new LegacyDomainNameReader(_reader, _buffer, current, _options, c => CheckMatchingCharacter(c));

			//Try to read the dns and act on the response.
			var state = reader.ReadDomainName();
			switch (state)
			{
				case LegacyDomainNameReader.ReaderNextState.ValidDomainName:
					return ReadEnd(ReadEndState.ValidUrl);
				case LegacyDomainNameReader.ReaderNextState.ReadFragment:
					return ReadFragment();
				case LegacyDomainNameReader.ReaderNextState.ReadPath:
					return ReadPath();
				case LegacyDomainNameReader.ReaderNextState.ReadPort:
					return ReadPort();
				case LegacyDomainNameReader.ReaderNextState.ReadUserPass:
					int host = _currentUrlMarker.IndexOf(LegacyUrlPart.HOST);
					_currentUrlMarker.UnsetIndex(LegacyUrlPart.HOST);
					return ReadUserPass(host);
				case LegacyDomainNameReader.ReaderNextState.ReadQueryString:
		...
		{
			_currentUrlMarker.SetIndex(LegacyUrlPart.FRAGMENT, _buffer.Length - 1);
		...
		{
			_currentUrlMarker.SetIndex(LegacyUrlPart.QUERY, _buffer.Length - 1);
		...
		{
			_currentUrlMarker.SetIndex(LegacyUrlPart.PORT, _buffer.Length);
			...
			while (!_reader.Eof())
			{
				...
				if (CheckMatchingCharacter(curr) == CharacterMatch.CharacterMatchStop || !LegacyCharUtils.IsNumeric(curr))
				{
					...
					_currentUrlMarker.UnsetIndex(LegacyUrlPart.PORT);
					return ReadEnd(ReadEndState.ValidUrl);
				}
		...
		{
			_currentUrlMarker.SetIndex(LegacyUrlPart.PATH, _buffer.Length - 1);
			while (!_reader.Eof())
			{
				//read the next char
				var curr = _reader.Read();
		...
		private bool ReadEnd(ReadEndState state)
		{
			...
			_currentUrlMarker = new LegacyUrlMarker();

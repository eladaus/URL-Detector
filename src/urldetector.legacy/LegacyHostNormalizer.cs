...
namespace urldetector.legacy
{
	...
	/// </summary>
	public class LegacyHostNormalizer
	{
		...
		public LegacyHostNormalizer(string host)
		{
			_host = host;
			Bytes = null;
		...
		private void NormalizeHost()
		{
			...
			try
			{
				host = LegacyAsciiTextEncodingExtensions.EncodeNonAsciiCharacters(_host);
			}
			catch (Exception)
			{
			...
			host = LegacyUrlUtil.Decode(host);
			...
			host = LegacyUrlUtil.RemoveExtraDots(host);

			_normalizedHost = LegacyUrlUtil.Encode(host).Replace("\\x", "%");
		}
		...
		{
			string[] parts = LegacyCharUtils.SplitByDot(host);
			...
			for (int i = 0; i < parts.Length; i++)
			{
				...
				else
				{
					try
					{
						...
						else if (8 == @base && LegacyOctalEncodingHelper.LooksLikeOctal(parsedNum.AsSpan()))
						{
		...
		private static bool IsHexSection(string section)
		{
			...
			{
				if (!LegacyCharUtils.IsHex(section[i]))
				{
					return false;
				}
			}

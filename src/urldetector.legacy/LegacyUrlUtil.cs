...
{
	public class LegacyUrlUtil
	{
		private LegacyUrlUtil()
		{
		}
		...
		public static string Decode(string url)
		{
			...
			while (i < stringBuilder.Length - 2)
			{
				...
				if (curr == '%')
				{
					if (LegacyCharUtils.IsHex(stringBuilder[i + 1]) && LegacyCharUtils.IsHex(stringBuilder[i + 2]))
					{
						var startIndex = i + 1;
						var endIndex = i + 3;
						...
						else if (!nonDecodedPercentIndices.IsEmpty() && LegacyCharUtils.IsHex(decodedChar)
						                                             && LegacyCharUtils.IsHex(stringBuilder[i - 1]) && i - nonDecodedPercentIndices.Peek() == 2)
						{
		...
		protected internal static string RemoveSpecialSpaces(string urlPart)
		{
			...
			{
				var curr = stringBuilder[i];
				if (LegacyCharUtils.IsWhiteSpace(curr))
				{
		...
		public static string RemoveExtraDots(string host)
		{
			...
			var reader = new LegacyInputTextReader(host);
			while (!reader.Eof())
			{
